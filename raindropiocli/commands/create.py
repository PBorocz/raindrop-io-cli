...
from raindropiocli.commands import (
    get_confirmation,
    get_from_list,
    get_title,
)
from raindropiocli.models.create_request import CreateRequest
...
def _add_bulk(el: EventLoop) -> None:
    while True:  # ie, Until we get a valid file..
        ...
        requests: list[CreateRequest] = [req for req in requests if _is_request_valid(el, req)]
        if not requests:
