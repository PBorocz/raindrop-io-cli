...
class RaindropState:
    ...
    # The timestamp associated with when we last refreshed from RaindropIO
    refreshed: datetime = None

    def get_collection_titles(self, exclude_unsorted=False, casefold=False) -> list[str]:
        """Return a sorted list of Collection titles, with or without 'Unsorted' and either casefolded or not."""
        if exclude_unsorted:
            titles = [collection.title for collection in self.collections if collection.id > 0 and collection.title]
        else:
            titles = [collection.title for collection in self.collections if collection.title]
    ...
    def refresh(self) -> bool:
        """Refresh the current state of this Raindrop environment (ie. current collections and tags)."""
        ...
        ################################################################################
        # Essentially, we want to determine if we need to perform the
        # somewhat time-consuming queries on behalf of the current
        # Collections and Tags. By saving the last known state of
        # these from a previous invocation, we may not have to query
        # for them.
        #
        # We determine this by comparing what essentially seems to be
        # a "lastUpdated" field (specifically:
        # "changedBookmarksDate")on the "meta" object coming back from
        # the SystemCollection. In interactive tests, any time we make
        # *any* modifying change to our RaindropIO environment, this
        # field will get a new timestamp.
        #
        # Thus, by comparing the -last- timestamp we had of this (from
        # our state file) against the -current- timestamp (by asking
        # RaindropIO for it), we can determine if we need to refresh
        # everything again.
        ################################################################################
        with Spinner("Refreshing Raindrop Status..."):
            ...
            if app_data_path := get_app_data_path():
                ...
                if state_path.exists():
                    ...
                    full_refresh = current_meta["changedBookmarksDate"] > self.meta["changedBookmarksDate"]
                    if False:
